...
Definition DRBG_instantiate_function 
  ...
  if Z.gtb requested_instantiation_security_strength highest_supported_security_strength
  then ENTROPY.error ENTROPY.generic_error entropy_stream
         ...
         | _ , _ =>
           if Z.gtb (Zlength personalization_string) max_personalization_string_length
           then ENTROPY.error ENTROPY.generic_error entropy_stream
           else
             let security_strength := requested_instantiation_security_strength in
             (* TODO actually follow specs *)
             (* here get_entropy returns min_entropy_length entropy *)
             match get_entropy security_strength min_entropy_length
                               max_entropy_length prediction_resistance_flag
                               entropy_stream with
               ...
               | ENTROPY.success entropy_input entropy_stream =>
                 ...
                 let initial_working_state :=
                     (* actual instantiate algorithm is called here *)
                     instantiate_algorithm entropy_input nonce
